...
static int setDPSconnectionInfo(const char * custom_dps_string, char ** uri, char ** id_scope)
{
   ...
   *             Typically it does not check if strstr() returns NULL so this can lead to malloc failures or inconsistent behaviors.
   */
  int ret = -1;
  char * prefix = NULL;
  char * value = NULL;
  ...
  *uri = calloc(size + 1, 1);
  ...
  {
    /* Get the ID scope */
    prefix = prefix+1;                          /* just after the previous ; */
    value = prefix + strlen(id_scope_prefix);   /* beginning of IdScope      */
    prefix = strstr(value,";");                 /* terminating ;             */
    size = prefix - value;
